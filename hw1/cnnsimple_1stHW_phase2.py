...
#model_type = random.choice([2,3,4]) # densenet cnn_bn  cnn_nobn cnn_nobn_multiconv
...
ttemp = random.sample([16, 32, 32,], 1)
ttemp1 = random.sample([32, 32, 64, 64, 128], 1)
ttemp2 = random.sample([64, 128, 64, 128, 64, 128, 64, 128], 2)
num_neuro = ttemp + ttemp1 + ttemp2
print(num_neuro)

num_conv = random.sample([1,1,2],1)+random.sample([1,1,2],1)+random.sample([1,1,2],1)+random.sample([1,1,2],1)
#num_conv = [1,1,1,1]
print("num_conv is ",num_conv)

acti_type = random.choice([1,2,3])
...
cnn_numlayer = random.choice([3,4])
print("the num of CNN layers is ",cnn_numlayer)

if cnn_numlayer == 4:
    fc_numlayer = random.choice([1, 1, 1, 1, 2])
    if fc_numlayer == 1:
        fc_num = random.sample([64, 128, 32], 1)
    else:
        fc_num = random.sample([256, 128, 128], 1) + random.sample([64, 32], 1)
else:
    fc_numlayer = random.choice([1,1,2,2,2])
    if fc_numlayer == 1:
        fc_num = random.sample([128, 256], 1)
    else:
        fc_num = random.sample([256, 512, 1024], 1) + random.sample([128, 64, 32], 1)

print("the num of FC layers is ",fc_numlayer)
print(fc_num)

BATCH_SIZE = random.choice([32,64,128])  ## minibatch size
...
if server == 0:
    ...
    mnistbasepath = "/Users/lishanyu/Desktop/pursuit_project/"
    savepath = "/Users/lishanyu/Desktop/pursuit_project/bestmodel.pkl"
else:
    import outer_pythoncode.data_pre as gendata
    datapath= '/data/user45/data_iceberg/'  ## the root path of train.json in server
    mnistbasepath = "/data/user45/pursuit_project/"
    savepath = "/data/user45/pursuit_project/bestmodel.pkl"
    ...
    def __init__(self):
        ...
        if num_conv[0] > 1:
            self.conv11 = nn.Sequential(
                nn.Conv2d(num_neuro[0], num_neuro[0], kernel_size=3, stride=1, padding=1, bias=False),
                nn.BatchNorm2d(num_neuro[0]),
                nn.ReLU(inplace=True)
            )
            if num_conv[0] == 3:
        ...
        if num_conv[1] > 1:
            self.conv21 = nn.Sequential(
                nn.Conv2d(num_neuro[1], num_neuro[1], kernel_size=3, stride=1, padding=1, bias=False),
                nn.BatchNorm2d(num_neuro[1]),
                nn.ReLU(inplace=True)
            )
            if num_conv[1] ==3 :
        ...
        if num_conv[2] > 1:
            self.conv31 = nn.Sequential(
                nn.Conv2d(num_neuro[2], num_neuro[2], kernel_size=3, stride=1, padding=1, bias=False),
                nn.BatchNorm2d(num_neuro[2]),
                nn.ReLU(inplace=True)
            )
            if num_conv[2] == 3:
        ...
        if cnn_numlayer==4:
            ...
            if num_conv[3] > 1:
                self.conv41 = nn.Sequential(
                    nn.Conv2d(num_neuro[3], num_neuro[3], kernel_size=3, stride=1, padding=1, bias=False),
                    nn.BatchNorm2d(num_neuro[3]),
                    nn.ReLU(inplace=True)
                )
                if num_conv[3] == 3:
        ...
        self.linear = nn.Sequential(
            nn.Linear(kk,fc_num[0]),
            nn.Dropout(0.2),
            our_acti()
        )
        if fc_numlayer == 1:
            self.fc = nn.Sequential(
                nn.Linear(fc_num[0], 1),
            )
        else:
            self.linear1 = nn.Sequential(
                nn.Linear(fc_num[0], fc_num[1]),
                nn.Dropout(0.3),
                our_acti()
            )
            self.fc = nn.Sequential(
                nn.Linear(fc_num[1],1),
            )
        self.sig = nn.Sigmoid()
        self.tan = nn.Tanh()
    def forward(self, x):
...
class CNNModel_noBN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(n_channel, num_neuro[0], kernel_size=3, stride=1, padding=0, bias = True),
            nn.ReLU(inplace=True)
        )
        if num_conv[0] > 1:
            self.conv11 = nn.Sequential(
                nn.Conv2d(num_neuro[0], num_neuro[0], kernel_size=3, stride=1, padding=1, bias=True),
                nn.ReLU(inplace=True)
            )
            if num_conv[0] == 3:
                self.conv12 = nn.Sequential(
        ...
        self.pool1 = nn.Sequential(nn.MaxPool2d(kernel_size=2,stride=2),nn.Dropout(0.2))
        self.conv2 = nn.Sequential(
            nn.Conv2d(num_neuro[0], num_neuro[1], kernel_size=3, stride=1, padding=0, bias = True),
            nn.ReLU(inplace=True)
        )
        if num_conv[1] > 1:
            self.conv21 = nn.Sequential(
                nn.Conv2d(num_neuro[1], num_neuro[1], kernel_size=3, stride=1, padding=1, bias=True),
                nn.ReLU(inplace=True)
            )
            if num_conv[1] ==3 :
                self.conv22 = nn.Sequential(
        ...
        self.pool2 = nn.Sequential(nn.MaxPool2d(kernel_size=2,stride=2),nn.Dropout(0.2))
        self.conv3 = nn.Sequential(
            nn.Conv2d(num_neuro[1], num_neuro[2], kernel_size=3, stride=1, padding=0, bias = True),
            nn.ReLU(inplace=True)
        )
        if num_conv[2] > 1:
            self.conv31 = nn.Sequential(
                nn.Conv2d(num_neuro[2], num_neuro[2], kernel_size=3, stride=1, padding=1, bias=True),
                nn.ReLU(inplace=True)
            )
            if num_conv[2] == 3:
                self.conv32 = nn.Sequential(
        ...
        if cnn_numlayer==4:
            self.conv4 = nn.Sequential(
                nn.Conv2d(num_neuro[2], num_neuro[3], kernel_size=3, stride=1, padding=0, bias = True),
                nn.ReLU(inplace=True)
            )
            if num_conv[3] > 1:
                self.conv41 = nn.Sequential(
                    nn.Conv2d(num_neuro[3], num_neuro[3], kernel_size=3, stride=1, padding=1, bias=True),
                    nn.ReLU(inplace=True)
                )
                if num_conv[3] == 3:
                    self.conv42 = nn.Sequential(
...
if __name__ == '__main__':
    ...
    for epoch in range(NUM_EPOCH):
        ...
        if tmp < best_val:
            ...
            if best_step < cur_step - kkk and best_val<0.17:
