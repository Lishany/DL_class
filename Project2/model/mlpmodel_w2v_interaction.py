...
if jieba:
    txtName = basepath + "lishan/resultmlpmodel_jiebapartuser.txt"
else:
...
class EDModel(nn.Module):
    def __init__(self, len_userdic,len_textdic,final_dim = 64,user_embedding_dim=50,enc_dim=40,text_embedding_dim = 50,time_embedding_dim = 10,num_layers = 1):
        ...
        self.mlpuser = nn.Sequential(nn.Linear(enc_dim,15),
                                nn.Dropout(0.3),nn.ReLU())
        self.mlptext = nn.Sequential(nn.Linear(self.text_embedding_dim+self.time_embedding_dim+5,15),
                                nn.Dropout(0.3),nn.Sigmoid())
        self.mlpuser2 = nn.Sequential(nn.Linear(enc_dim,3),
                                nn.Dropout(0.3),nn.ReLU())
        self.mlpout = nn.Sequential(nn.Linear(15,3),
    ...
    def forward(self, sen,user,time,seq_lengths,num4,textnum):
        ...
        #label3 = self.mlpout(all_out)
        all_out = all_out.view((sen.size()[0],3,-1))
        label3 = torch.sum(all_out,2)/5
