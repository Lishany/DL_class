...
if jieba:
    txtName = basepath + "lishan/mlpmodel_plus.txt"
else:
...
class EDModel(nn.Module):
    ...
    def forward(self, sen,user,time,seq_lengths,num4,textnum):
        ...
        sen_embeds = torch.cat((sen_embeds,textnum,time_embeds),1)
...
for ep in range(n_epoches):
    ...
    if isval:
        ...
        for i, (bsen, seq_lengths, buser, btime, ll, num4,textnum) in enumerate(val_loader,1):
            ...
            accuracy += accu.data
            if i %10 == 0:
                print("loss1 ",allloss1.cpu().numpy().item()/10,"loss ", all_loss.cpu().numpy().item() / 10,"a acc ", accuracy.cpu().numpy().item()/ 10)
