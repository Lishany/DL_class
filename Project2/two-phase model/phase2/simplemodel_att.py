...
if jieba:
    txtName = basepath + 'phase2/resultrnn+att_file'+str(file_num)+'.txt'
...
class EDModel(nn.Module):
    def __init__(self, len_userdic,len_textdic,hidden_dim2 = 80,final_dim = 50,user_embedding_dim=60,enc_dim=50,text_embedding_dim = 100,time_embedding_dim = 10,num_layers = 1):
        ...
        self.attmlp = nn.Sequential(nn.Linear(hidden_dim2+text_embedding_dim,32),
                                nn.Dropout(0.3),nn.Tanh(),nn.Linear(32,1),
                                nn.Dropout(0.3))
    ...
    def forward(self, sen,user,time,seq_lengths,num4,textnum):
        ...
        out_vec2 = out_vec2.unsqueeze(1)
        out_vec2 = out_vec2.repeat(1,text_embeds.size()[1],1)
        input_att = torch.cat((out_vec2,text_embeds),2)
        att = self.attmlp(input_att)
        att = att.view(sen.size()[0],-1)
        att = F.softmax(att)
        att = att.unsqueeze(2)
        sen_embeds = text_embeds*att
...
import numpy as np
if jieba:
    datapath = basepath + 'train_jeiba7/train_jeiba'+file_num+'.json'
