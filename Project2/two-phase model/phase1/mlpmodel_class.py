...
if jieba:
    txtName = basepath + "lishan/class_resultmlpmodel_plus.txt"

class EDModel(nn.Module):
    def __init__(self, len_userdic,len_textdic,final_dim = 64,user_embedding_dim=60,enc_dim=40,text_embedding_dim = 50,time_embedding_dim = 10,num_layers = 1):
        ...
        self.mlp = nn.Sequential(nn.Linear(text_embedding_dim+enc_dim+17+5,final_dim),
                                nn.Dropout(0.3),nn.ReLU())
        self.mlp1 = nn.Sequential(nn.Linear(final_dim, 8),
                                     nn.Dropout(0.3))
        self.coef = nn.Sequential(nn.Linear(text_embedding_dim+enc_dim+17+5,8),
                                nn.Dropout(0.3),nn.Sigmoid())
        self.mlp2 = nn.Sequential(nn.Linear(text_embedding_dim+enc_dim+17+5,8),
                                nn.Dropout(0.3),nn.ReLU())
    def forward(self, sen,user,time,seq_lengths,num4,textnum):
        ...
        out1 = self.mlp2(user_text)
        label3 = self.mlp1(label3)
        coef = self.coef(user_text)
        label3 = label3*coef+out1
        return F.log_softmax(label3,dim=1)

#loss_pred = Loss_3()
loss_pred = nn.NLLLoss()
import pandas as pd
import numpy as np
...
for ep in range(n_epoches):
    ...
    for i, (bsen, seq_lengths, buser, btime, ll,num4,textnum) in enumerate(train_loader,1):
        ...
        pred = label3.data.max(1,keepdim = True)[1]
    ...
    if isval:
        ...
        if ep == n_epoches-1:
            ...
            for i, (bsen, seq_lengths, buser, btime, ll, num4,textnum) in enumerate(val_loader,1):
                ...
                label3 = label3.data.max(1,keepdim = True)[1]
            ...
            f = open(basepath + "phase1/validation_mlp_class.txt", "w+")
            i = 0
...
if save:
    ...
    f = open(basepath + "phase1/prediction_mlp_class.txt", "w+")
    i = 0
    for i in range(sen_result.shape[0]):
